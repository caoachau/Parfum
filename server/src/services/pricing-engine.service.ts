...
export const EXPRESS_SHIPPING_FEE = 35_000;

export function shippingFeeFor(method: ShippingMethod = 'standard'): number {
  return method === 'express' ? EXPRESS_SHIPPING_FEE : 0;
}

/** Hoan tat tong tien theo semantics gia da gom VAT. */
export function finalizeVatInclusiveTotal(
  subtotal: number,
  voucherDiscount: number,
  shippingFee: number,
) {
  const productTotal = Math.max(0, subtotal);
  const finalTotal = Math.max(0, productTotal - voucherDiscount + shippingFee);
  const vatRate = VAT_RATE;
  return {
    finalTotal,
    vatRate,
    // VAT la snapshot cua gia san pham. Voucher o cap don hang va phi van chuyen
    // chi thay doi so tien thanh toan, khong lam thay doi VAT trong gia san pham.
    vatIncluded: calculateVatIncluded(productTotal, vatRate),
...
export async function resolveVariantPrices(variants: any[], now = new Date()) {
  ...
  const categoryIds = variants
    .map((variant) => idOf(variant._categoryId || variant.product?.category))
    .filter(Boolean);
  ...
  for (const variant of variants) {
    ...
    if (flash) {
      ...
      result.set(variantId, {
        ...
        discountPercent: referencePrice
    ...
    if (discount) {
      ...
      result.set(variantId, {
        ...
        promotionId: String(discount._id),
...
async function customerSegment(userId?: string) {
  ...
  const paid: any[] = await Payment.find({ order: { $in: orderIds }, status: 'paid' })
    .select('amount')
    .lean();
...
async function resolveVoucher(
  ...
  if (!voucher)
    throw Object.assign(new Error('Mã ưu đãi không tồn tại hoặc đã tắt'), { status: 400 });
  const start = voucher.startDate ? new Date(voucher.startDate) : null;
  const end =
    voucher.endDate || voucher.expiresAt ? new Date(voucher.endDate || voucher.expiresAt) : null;
  if (start && start > now)
    throw Object.assign(new Error('Mã ưu đãi chưa đến thời gian sử dụng'), { status: 400 });
  ...
  if (subtotal < minOrder)
    throw Object.assign(
      new Error(`Đơn hàng cần đạt tối thiểu ${minOrder.toLocaleString('vi-VN')}đ`),
      { status: 400 },
    );
  ...
  if (!mayStackVoucher(voucher.stackable !== false, hasProductPromotion ? 1 : 0)) {
    throw Object.assign(new Error('Mã này không thể dùng chung với ưu đãi sản phẩm hiện có'), {
      status: 400,
    });
  }
  ...
  if (voucher.appliesToNewMembers) {
    if (!userId || !Types.ObjectId.isValid(userId)) {
      throw Object.assign(new Error('Vui lòng đăng nhập để dùng voucher dành cho thành viên mới'), {
        status: 400,
      });
    }
    ...
    if (!assignedUser || existingOrder) {
      throw Object.assign(
        new Error('Voucher này chỉ dành cho tài khoản mới chưa có đơn hàng và đã cập nhật hồ sơ'),
        { status: 400 },
      );
    }
  }
  if (actualSegment === 'GUEST' && voucher.userSegment && voucher.userSegment !== 'ALL') {
    throw Object.assign(
      new Error('Vui lòng đăng ký hoặc đăng nhập để dùng mã ưu đãi thành viên mới'),
      { status: 400 },
    );
  }
  if (!segmentMatches(voucher.userSegment, actualSegment)) {
    throw Object.assign(new Error('Mã ưu đãi không áp dụng cho nhóm khách hàng của bạn'), {
      status: 400,
    });
  }
  const key = customerKey(userId, email);
  if (voucher.usageLimitPerUser > 0 && key) {
    const counter: any = await VoucherCounter.findOne({
      voucher: voucher._id,
      customerKey: key,
    }).lean();
    if (Number(counter?.count || 0) >= voucher.usageLimitPerUser) {
      throw Object.assign(new Error('Bạn đã sử dụng hết số lượt của mã ưu đãi này'), {
        status: 400,
      });
    }
  }
...
export async function quoteOrder(
  ...
  const variants: any[] = await Variant.find({
    _id: { $in: clean.map((item) => item.variant) },
    isActive: { $ne: false },
  })
    .populate({
      path: 'product',
  ...
  const quotedItems = clean.map((item) => {
    ...
    const available =
      price.flashRemaining == null
        ? Number(variant.stock || 0)
        : Math.min(Number(variant.stock || 0), price.flashRemaining);
    if (available < item.quantity) {
      throw Object.assign(
        new Error(`Sản phẩm ${variant.product?.name || variant.sku} chỉ còn ${available}`),
        { status: 409, variant: item.variant },
      );
    }
    return {
      ...price,
  ...
  const voucher: any = await resolveVoucher(
    options.voucherCode,
    subtotal,
  ...
  if (voucher && normalizedType !== 'free_shipping') {
    ...
    if (!voucher.isConcentratedPromotion)
      voucherDiscount = Math.min(voucherDiscount, Math.round(subtotal * 0.5));
  }
  const shippingFeeBeforeDiscount = shippingFeeFor(options.shippingMethod);
  const shippingDiscount =
    voucher && normalizedType === 'free_shipping' ? shippingFeeBeforeDiscount : 0;
  const shippingFee = shippingFeeBeforeDiscount - shippingDiscount;
  // VAT da nam trong gia san pham. finalTotal khong cong them thue; voucher o cap
  // don hang va phi van chuyen khong lam thay doi snapshot VAT cua san pham.
  const { finalTotal, vatRate, vatIncluded, pricesIncludeVat } = finalizeVatInclusiveTotal(
    subtotal,
    voucherDiscount,
    shippingFee,
  );
  return {
    ...
    voucher: voucher
      ? {
          ...
          userSegment: voucher.userSegment || 'ALL',
